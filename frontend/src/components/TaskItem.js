...
export default function TaskItem({ task, onPress }) {
  ...
  // 2. LOGIKA TERLAMBAT (Hanya jika tanggal KEMARIN atau sebelumnya)
  const checkOverdue = () => {
    ...
    dueDate.setHours(0, 0, 0, 0); // Reset jam deadline ke 00:00

    // Overdue hanya jika deadline < hari ini (Kemarin dst)
  ...
  return (
    <TouchableOpacity activeOpacity={0.9} onPress={onPress} style={styles.container}>
      <View style={[styles.card, isOverdue && styles.cardOverdue]}>
        <LinearGradient colors={priorityConfig.gradient} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={styles.borderGradient} />
        <View style={styles.content}>
          ...
          <View style={styles.footer}>
            <View style={styles.dateRow}>
              <Ionicons name={isOverdue ? "alert-circle" : "calendar-outline"} size={16} color={isOverdue ? theme.colors.danger : theme.colors.textMuted} />
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: theme.colors.card, borderRadius: theme.radius.l, flexDirection: 'row', overflow: 'hidden', ...theme.shadow.small, borderWidth: 1, borderColor: 'transparent' },
  cardOverdue: { borderColor: theme.colors.danger + '40' },
  borderGradient: { width: 6 },
  content: { flex: 1, padding: 16 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  categoryBadge: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#EFF6FF', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, maxWidth: '60%' },
  ...
  footer: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: 12, borderTopWidth: 1, borderTopColor: theme.colors.divider },
